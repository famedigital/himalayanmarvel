...
export default function TripAdvisorReviewsEditor({
  ...
  const handleSave = async () => {
    ...
    await onSave(data);
    setSaving(false);
    toast.success('Reviews section saved');
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400">
            Manage rating display and top reviews
          </p>
        </div>
        <button
          onClick={handleSave}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
            Review Source (Google, TripAdvisor, etc.)
          </label>
          <input
            type="text"
            value={data.reviewSource || 'Google'}
            onChange={(e) => setData({ ...data, reviewSource: e.target.value })}
            className="w-full px-4 py-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none"
            placeholder="Google"
          />
        </div>
