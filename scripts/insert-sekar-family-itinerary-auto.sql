-- Insert Sekar Family Itinerary - ALL IN ONE SCRIPT
-- This uses a CTE to insert everything in a single transaction
...
WITH new_itinerary AS (
  INSERT INTO itineraries (
    ...
    '{
      ...
      "items": [
        {"label": "Luxury Accommodation", "description": "Ama''s Farmstay Guma & Paro (4 nights) + Premium Hotels (2 nights) + Phuentsholing (1 night)", "amount": ""},
        {"label": "Meals", "description": "Daily breakfast & dinner (MAP plan)", "amount": ""},
        {"label": "Premium Transport", "description": "Hyundai Santa Fe or similar SUV with experienced driver", "amount": ""},
        {"label": "English-Speaking Guide", "description": "Licensed, knowledgeable guide throughout Bhutan", "amount": ""}
      ],
      "inclusions": [
      ...
      "exclusions": [
        "Lunch and snacks during journey",
    ...
    '{
      "booking_payment": "A 30% non-refundable deposit is required to confirm your booking. The remaining balance must be paid 30 days before your arrival date. We accept wire transfers, UPI, and all major credit cards. For Indian guests, payments can be made in INR equivalent.",
      "cancellation_policy": "Full refund (minus deposit) for cancellations made 60+ days before departure. 50% refund for cancellations 30-60 days before. No refund within 30 days of departure unless covered by travel insurance.",
      "travel_insurance": "Travel insurance is recommended for all travelers.",
      "health_fitness": "A reasonable level of fitness is required for the Tiger''s Nest trek. We recommend consulting your doctor before undertaking high-altitude trekking. Inform us of any medical conditions in advance.",
      "liability": "Silverpine Bhutan acts as an agent for hotels, transport, and service providers. We shall not be liable for any injury, damage, loss, delay, or irregularity caused by defects in services or acts of God."
    }'::jsonb,
    '{
      "documents": ["Valid Passports (6 months validity)", "Voter ID (for Indian nationals)", "Travel insurance documents", "Flight tickets (QP-1850/QP-1851)", "Passport size photos (4 copies)", "Children''s school ID/birth certificate", "Emergency contact numbers", "Medical prescriptions"],
      "clothing": ["Comfortable walking shoes", "Light woolens/sweaters", "Windproof jacket", "Casual cotton clothes", "Sunglasses & sun hat", "Sturdy trekking shoes", "Warm jacket/sweater", "Comfortable pants (no jeans for trek)", "Cap/hat", "Extra layer for evenings"],
      "gear": ["Sturdy hiking shoes", "Comfortable socks (2 pairs)", "Light backpack", "Water bottle (1L)", "Sunscreen & hat", "Light rain jacket", "Trekking poles (optional)", "Energy snacks", "Camera/phone charger", "Small first aid kit", "Wet wipes", "Hand sanitizer", "Sunscreen SPF 50+", "Lip balm", "Moisturizer", "Reusable water bottles"],
      "essentials": ["Universal adapters (230V)", "Personal medications", "Altitude sickness meds", "Pain relievers", "Band-aids / blister kit"]
    }'::jsonb,
    '{
      ...
      "background_image": "https://images.unsplash.com/photo-1578301978018-3005759f48f7?q=80&w=2000"
    }'::jsonb,
    '{
      "header_left": "Silverpine Bhutan",
      "header_right_title": "Bhutan Family Discovery",
...
SELECT
  ...
  unnest(ARRAY[1, 2, 3, 4, 5, 6, 7]::INTEGER[]),
  unnest(ARRAY[
  ...
  unnest(ARRAY[NULL, NULL, NULL, NULL, '3,120m', NULL, NULL]::TEXT[]),
  unnest(ARRAY['~4 hours drive', NULL, '~3 hours total', NULL, '~5km', '5 hours', '~4 hours']::TEXT[]),
  unnest(ARRAY['Full day', 'Full day', 'Full day', 'Leisure day', '4-5 hours', 'Full day', 'Full day']::TEXT[]),
  unnest(ARRAY[NULL, NULL, NULL, NULL, 'Tiger''s Nest Monastery', NULL, NULL]::TEXT[]),
  unnest(ARRAY['Thimphu', 'Thimphu', 'Ama''s Farmstay, Guma (near Punakha)', 'Ama''s Farmstay', 'Ama''s Farmstay', 'Phuentsholing', NULL]::TEXT[]),
  unnest(ARRAY[
  ...
  unnest(ARRAY[true, true, true, true, true, true, false]::BOOLEAN[]),
  unnest(ARRAY['Hotel buffet', 'Hotel buffet', 'Hotel Thimphu', 'Ama''s Farmstay', 'Early energizing breakfast', 'Farmstay breakfast', 'Phuentsholing hotel']::TEXT[]),
  unnest(ARRAY['En route', 'Local restaurant – try Momos!', 'Ama''s Farmstay (₹600/person) OR Local restaurant – decide in morning!', 'Paro town restaurant', 'Taktsang cafeteria with mountain views', 'En route', NULL]::TEXT[]),
  unnest(ARRAY['Hotel dinner', 'Bhutanese specialty dinner', 'Ama''s Farmstay – traditional Bhutanese', 'Farmstay dinner with local flavors', 'Celebration dinner at farmstay', 'Hotel dinner', NULL]::TEXT[]),
  unnest(ARRAY[NULL, NULL, NULL, NULL, NULL, NULL, NULL]::TEXT[]),
  unnest(ARRAY[NULL, NULL, NULL, NULL, NULL, NULL, NULL]::TEXT[]),
  unnest(ARRAY[NULL, NULL, NULL, NULL, NULL, NULL, NULL]::TEXT[]),
  unnest(ARRAY[
  ...
  unnest(ARRAY['Himalayan Mountains', 'Buddha Dordenma', 'Ama''s Farmstay', 'Ama''s Farmstay Paro', 'Tiger''s Nest', 'Phuentsholing Border', 'Farewell']::TEXT[]),
  unnest(ARRAY[
...
INSERT INTO itinerary_section_openers (
  itinerary_id,
  section_number,
  title,
  subtitle,
...
SELECT
  (SELECT id FROM new_itinerary),
  unnest(ARRAY[1, 2, 3, 4, 5]::INTEGER[]),
  unnest(ARRAY['THE JOURNEY BEGINS', 'THIMPHU DISCOVERY', 'VALLEYS OF WONDER', 'PARO VALLEY', 'THE SACRED HIKE']::TEXT[]),
  unnest(ARRAY[
    'Bangalore • Bagdogra • Thimphu',
    'Cultural Exploration Day',
    'Punakha • Guma • Ama''s Farmstay',
    'Traditional Town & Culture',
    'Tiger''s Nest • A Spiritual Journey'
  ]::TEXT[]),
  unnest(ARRAY[
  ...
  unnest(ARRAY['rgba(45,90,61,0.6)', 'rgba(45,90,61,0.6)', 'rgba(45,90,61,0.6)', 'rgba(45,90,61,0.6)', 'rgba(45,90,61,0.6)']::TEXT[]),
  unnest(ARRAY[4, 6, 8, 10, 12]::INTEGER[]);
